...
#     X, Y, Z = pc
#     if Z <= 1e-12: return np.nan, np.nan
...
#         if g <= 1e-12: g = 1e-12
#         Delta = np.sqrt(g)
#         ru = np.sqrt(max(ru2, 1e-24))
#         rd = (1.0 - Delta) / (2.0*k*ru)
#         s = rd / (ru + 1e-12)
#         xd, yd = s*ux, s*uy
#     return xd/sx + cx, yd/sy + cy

def _print_proj_debug(tag, T_Board_Cam, obj_pts, intr):
    # 보드 점들을 카메라 좌표로
    pts_cam = (T_Board_Cam[:3,:3] @ obj_pts.T + T_Board_Cam[:3,3,None]).T
    z = pts_cam[:,2]
    uv, mask = project_division_model(obj_pts, T_Board_Cam, intr)
    print(f"[DBG {tag}] Z>0 ratio={np.mean(z>0):.2f}, Z[min,max]=({z.min():.3f},{z.max():.3f}) "
          f"| finite={np.mean(np.isfinite(uv).all(1)):.2f}, vis_ratio={np.mean(mask):.2f}")
...
def simulate_and_save_realistic_data(
    ...
    *,
    ...
    for i, f in enumerate(frames):
        ...
        projgt_px, mgt = project_division_model(obj_pts, B_gt, intr1_gt)
        B_est = X_gt @ A_meas @ T_Board_B1
        proj1_px, m1 = project_division_model(obj_pts, B_est, intr1_gt)
        # proj2_px, m2 = project_division_model(obj_pts, T_BC2_est, intr2_est)

        egt = f['img1_meas'][mgt] - projgt_px[mgt]
        e1 = f['img1_meas'][m1] - proj1_px[m1]
        # e2 = f['img2_meas'][m2] - proj2_px[m2]
        rmsegt = float(np.sqrt(np.mean(np.sum(egt**2, axis=1)))) if mgt.any() else np.nan
        ...
        # print(f"[Frame {i:03d}] cam1: rot={rdeg1:6.3f} deg, trans={tmm1:7.3f} mm, RMSE={rmse1:7.3f} px | "
        #       f"cam2: rot={rdeg2:6.3f} deg, trans={tmm2:7.3f} mm, RMSE={rmse2:7.3f} px")

        # 🔎 GT 체인 잔차:  AXB ?= YCZD
        #  - 스코프에 X_gt, Y_gt, Z_gt가 없다면 f['X_gt'] 등으로 바꿔 써도 됩니다.
        T_L = A_meas @ T_Board_B1
        T_R = inv4(X_gt) @ T_C1B_pnp
        ...
        print(
            ...
            f"RMSE(gt)={rmsegt:7.3f} px, RMSE(noise)={rmse1:7.3f} px | "
            f"AX vs ZB: ΔR={chain_rdeg:6.3f} deg, Δt={chain_tmm:7.3f} mm"
        )
        ...
        # YAML 저장 (파이프라인 호환)
        gt_blob = {"GT": {"X": X_gt.tolist(), "Z": Z_gt.tolist(), "Y": Y_gt.tolist(), "B": T_Board_B1.tolist(), "T_Base_to_EE": f['A_gt'].tolist()}}
        data1 = {
...
if __name__ == "__main__":
    ...
    Z_gt = se3(
        so3_exp([-0.01,  0.04, -0.02]),   # ~ -0.6°,  2.3°, -1.1°
        [0.06, 0.00, 0.10]
    )
